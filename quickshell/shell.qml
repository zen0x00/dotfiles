...
ShellRoot {
    ...
    function applyWallpaper(wallpaper) {
        ...
        applyWallProc.command = ["bash", "-c", "ln -sf '" + wallpaper.path + "' '" + root.wallpaperPath + "/current' && " + "swww img '" + wallpaper.path + "' --transition-type any --transition-duration 2 & " + "wal -i '" + wallpaper.path + "' -n -q && " + "sleep 0.3"];
    ...
    Process {
        ...
        command: ["bash", "-c", "THUMB_DIR='" + root.cachePath + "/wallpaper-thumbs' && " + "WALL_DIR='" + root.wallpaperPath + "' && " + "cd \"$THUMB_DIR\" && " + "find \"$WALL_DIR\" -maxdepth 1 -type f \\( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.gif' -o -iname '*.png' -o -iname '*.webp' \\) ! -name '.*' 2>/dev/null | " + "while IFS= read -r f; do " + "  hash=$(echo -n \"$f\" | md5sum | cut -d' ' -f1); " + "  thumb=\"$THUMB_DIR/${hash}.jpg\"; " + "  if [ ! -f \"$thumb\" ] || [ \"$f\" -nt \"$thumb\" ]; then " + "    if command -v vipsthumbnail >/dev/null 2>&1; then " + "      case \"$f\" in " + "        *.gif) convert \"${f}[0]\" -thumbnail 180x120^ -gravity center -extent 180x120 -quality 85 \"$thumb\" 2>/dev/null ;; " + "        *) vipsthumbnail \"$f\" -s 180x120 -o \"$thumb\" 2>/dev/null || convert \"$f\" -thumbnail 180x120^ -gravity center -extent 180x120 -quality 85 \"$thumb\" 2>/dev/null ;; " + "      esac; " + "    else " + "      case \"$f\" in " + "        *.gif) convert \"${f}[0]\" -thumbnail 180x120^ -gravity center -extent 180x120 -quality 85 \"$thumb\" 2>/dev/null ;; " + "        *) convert \"$f\" -thumbnail 180x120^ -gravity center -extent 180x120 -quality 85 \"$thumb\" 2>/dev/null ;; " + "      esac; " + "    fi; " + "  fi; " + "done"]
        onExited: {
    ...
    Process {
        ...
        onExited: {
            if (root.walApplying) {
                if (!walStepSwaync.running)
                    walStepSwaync.running = true;
    ...
    Process {
        ...
        onExited: {
            if (!walStepBlur.running)
                walStepBlur.running = true;
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (data) => {
                return root.currentWallpaper = data.trim();
            }
        }
    ...
    Process {
        ...
        command: ["bash", "-c", "for f in /usr/share/applications/*.desktop '" + root.homePath + "/.local/share/applications'/*.desktop; do " + "  [ -f \"$f\" ] || continue; " + "  grep -qi '^NoDisplay=true' \"$f\" && continue; " + "  grep -qi '^Hidden=true' \"$f\" && continue; " + "  name=$(grep -m1 '^Name=' \"$f\" | cut -d= -f2-); " + "  exec=$(grep -m1 '^Exec=' \"$f\" | cut -d= -f2- | sed 's/ %[fFuUdDnNickvm]//g'); " + "  icon=$(grep -m1 '^Icon=' \"$f\" | cut -d= -f2-); " + "  [ -z \"$name\" ] && continue; " + "  [ -z \"$exec\" ] && continue; " + "  printf '%s\\t%s\\t%s\\n' \"$name\" \"$exec\" \"$icon\"; " + "done | sort -f -t$'\\t' -k1,1 | awk -F'\\t' '!seen[$1]++'"]
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (data) => {
                ...
                var ssid = parts[0];
                if (ssid === "" || ssid === root.wifiCurrentSSID)
                    return ;
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (data) => {
                return root.btEnabled = data.trim() === "true";
            }
        }
